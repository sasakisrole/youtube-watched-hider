...
拡張を更新してから **照合 → 1件だけ削除 → 照合**。2回目の照合に `前回比 …` が出る。
`削除ID変化 0件` なら「振り直されない」、1件でも変化していれば「振り直される」。
...
3. Round D: **まず上記の計測を実機で1回**。結果を見てから、まとめて削除の設計
   （削除直前の再照合をどこまでやるか・古い `setVideoId` の再取得・曖昧時停止）を決める。
   削除対象一覧を出す画面（I-044）もここで要る（10件超の一括操作は事前に全件提示するため）。
