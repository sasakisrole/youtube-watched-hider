# 後で見る（Watch Later）一括整理 — Round A〜D 実装メモ

- 起点: ChatGPT引き継ぎ `claude-code-handoff-20260807-230154-youtube-watch-later-cleanup.md`（I-001〜I-102）
- 実装日: Round A/B = 2026-08-07 ／ Round C/D = 2026-08-08（v1.43.12）
- 状態: **照合・1件削除・まとめて削除まで完了・実機受入済み**。残るのは失敗経路の確認（Round E/F）

## これは何をするものか

「後で見る」に溜まった動画のうち、Watched Hider が視聴済みとして持っている動画を
まとめて消すための機能。**照合（数えるだけ）／1件ずつの削除／一覧を確認してまとめて削除**
の3つが使える。
...
| `tests/verify_watch_later_core.js` | 75 → **112 チェック** |

テストは `tests/verify_*.js` **全25本 PASS**。

### 実機受入（2026-08-08・v1.43.12）

既定の5件で実行:

```text
5件を削除しました / 残り: 後で見る 573件 / 視聴済み一致 36件 / 未視聴 537件
```

| 項目 | 前 | 後 | 差 |
|---|---|---|---|
| 後で見る | 578 | 573 | **−5**（指定した件数と一致） |
| 視聴済み一致 | 41 | 36 | **−5** |
| **未視聴** | **537** | **537** | **±0** |

中止理由・`削除ID変化` の警告・再照合失敗のいずれも出ていない＝5件とも正常終了。

⚠️ **5件はチャンク境界（10件）に届いていない**ので、チャンクごとの再照合とドリフト停止は
実機では一度も走っていない。10件を超える実行で初めて通る経路。

### 途中で見つけて直した不具合（v1.43.12）

初回の実行で画面が「削除中…」のまま固まり、1件も削除されなかった。原因は
`history.js` が Port に接続しただけで **START メッセージを送っていなかった**こと。
background は開始待ち、Port は開いたままで切断も検知されず、画面が永久に待ち続けていた。

**この不具合をテストが素通りさせた理由**のほうが学びとして重い:

- 既存の source pin は `background.js` しか見ておらず、**画面と worker の受け渡しが
  食い違ってもどちらのファイルも単体では壊れない**
- 追加したピンのうち1本は、`history.js` 内の**別機能3つ（fix-channels / fix-credits /
  fix-durations）が同じ `type: 'START'` を送っている**ため誤って合格していた。
  バッチの `connect` から次の `connect` までに範囲を絞って修正
- 感応性を確認済み: 開始メッセージ削除で2件 FAIL・メッセージ名の食い違いで2件 FAIL・
  復元後 112 passed（復元はバイト単位で SHA-256 一致を確認）

∴ **同一ファイル内で同じ文字列を使う機能が複数あるとき、source pin は範囲を絞らないと
偽陽性で合格する。** 同種の契約（Port 名と開始メッセージ）は他機能にも残っている。
...
3. ~~Round D~~ → 2026-08-08 計測・実装・テスト・**実機受入まで完了**（上記3節）。
   削除対象一覧（I-044）も実装。v1.43.12 として版に反映。
4. Round E/F: 中止・通信失敗・他端末変更、privacy説明の更新。
   実機で一度も踏んでいない経路: **チャンク境界（10件超）での再照合とドリフト停止**／
   アカウント切替／照合の10分超過／`STATUS_SUCCEEDED` 以外の応答／無応答時の打ち切り。
   残り36件をまとめて削除すればチャンク境界は自然に3回通る。
