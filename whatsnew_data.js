...
globalThis.YWH_WHATSNEW = [
  {
    "version": "1.52.0",
    "date": "2026-09-06",
    "summary": "履歴画面の取り違え・見落としを減らす手当てをまとめて入れた。一番大きいのは、動画1件の削除に「元に戻す」猶予を付けたこと。ほかは、キーボードで操作したときに見えないボタン、押せない理由が読めないボタン、絞り込むと消える母数、英語のまま残っていた文言、長い一覧で先頭へ戻る手段。できることの追加は「元に戻す」と「先頭へ戻る」の2つで、削除の対象や条件は変えていない。新しい権限は要求していない。",
    "points": [
      ...
      "あわせて、削除で一覧が1件減ったときに次の100件の先頭が1件ぶん飛んで表示されなくなる問題も直した（描画済み件数を一緒に詰めていなかった）。",
      "キーボードで削除ボタンへ移動したとき、ボタンが見えるようにした。× はマウスを乗せたときだけ出る作りで、Tab で移動すると「見えないボタンにフォーカスがある」状態だった。フォーカスが行にあるときも表示し、フォーカス枠も出す。",
      "押せないボタンの理由が読めるようにした。「1件だけ削除」「まとめて削除」は照合前は押せないが、押せない状態のときだけ説明のツールチップが出ない作りだった（pointer-events を落としていたため）。なぜ押せないかの説明が、押せないときだけ読めないという状態を解消した。",
      "絞り込み中も全体の件数が読めるようにした。検索すると右上の件数が絞り込み後の数に置き換わり、母数が分からなくなっていた。「128 件（全33,412件）」の形にした。",
      "英語のまま残っていた画面文言を日本語にした（4か所）。× の説明、視聴を検出した経路のバッジ2種、そして読み込みに失敗したときのメッセージ。一番困っているときに出る文が英語だった。",
      "「先頭へ戻る」ボタンを足した。600px 以上スクロールすると右下に出る。100件ずつ読み込む一覧なので、下へ進むほど先頭が遠かった。",
      "保存しているデータの構造（DB_VERSION）は 5 のままで、既存のデータを作り替える処理も行わない。外部への送信も増えていない（permissions / host_permissions は v1.51.0 と同一）。",
      "Test: 回帰ハーネス45本 0 failed（今回 tests/verify_history_delete_undo.js を新設して36件を追加）。新設分は出荷版 history.js の削除ブロックをそのまま実行し、DELETE_VIDEO が実際に何件・どのIDで送られたかを見ている。感応性＝描画済み件数を詰めない・押した瞬間に実削除する・失敗しても戻さない・タブを閉じるときに送らない・隠した行の display 打ち消しを外す、の5変異でそれぞれ対応する検査だけが FAIL し、戻すと全通過。",
  ...
  },
  {
    "version": "1.51.0",
    "date": "2026-09-06",
    "summary": "「後で見る」のまとめて削除を、件数の決め方の面で軽くした。削除する動画の一覧を読んだあと、件数欄まで手を動かして数字を打ち直す手間と、「一覧に出た分をとにかく全部消したい」ときに件数を数えて入力する手間を減らした。削除の仕組み・順序・安全確認は変えていない。新しい権限は要求していない。",
    "points": [
      ...
      "保存しているデータの構造（DB_VERSION）は 5 のままで、既存のデータを作り替える処理も行わない。外部への送信も増えていない（permissions / host_permissions は v1.50.0 と同一）。",
      "Test: 回帰ハーネス44本 0 failed（今回 tests/verify_watch_later_panel_ui.js を新設して22件を追加）。新設分は出荷版のソースから attachWheelStepper と startWatchLaterBatch を切り出して実行し、実際に START へ乗る件数を読んでいる（ソース文字列の一致ではないので、渡す値が入れ替われば落ちる）。感応性＝passive: false を外す・max の頭打ちを外す・全件ボタンが件数欄の値を使うように変える、の3変異でそれぞれ対応する検査だけが FAIL し、戻すと全通過。",
  ...
  },
  {
    "version": "1.50.0",
    "date": "2026-09-03",
    "summary": "寄付の導線を1本だけ足した。無償で公開しているツールなので、役に立った人が任意で支援できる先へのリンクを、ポップアップの一番下と「使い方・更新情報」ページの末尾に置いた。機能の追加・削除はなく、新しい権限も要求していない。",
    "points": [
  ...
  },
  {
    "version": "1.49.0",
    "date": "2026-08-26",
    "summary": "「後で見る」の整理まわりの使い勝手を直した。よく使う3つの操作が「データ管理・一括処理」の折り畳みの中にあり、削除する動画の一覧は折り畳みの外に出ていたため、一覧を読むには折り畳みを閉じる必要があった。しかも削除の実行中は折り畳みを閉じられない作りだったので、一番読みたいときに一番読めない状態だった。できることの追加・削除はなく、置き場所と見え方の変更だけ。",
    "points": [
      ...
      "Test: 回帰ハーネス36本 0 failed。画面の受入は、既存の37件に加えて今回20件を追加して全通過（データ管理を開いたままでも一覧が画面内に収まること、キーボードでの到達とフォーカス枠、Escape・背景での開閉、削除中は閉じないこと、200%相当の幅で崩れないこと、ライト・ダーク双方）。",
  ...
  },
  {
    "version": "1.48.0",
    "date": "2026-08-26",
    "summary": "クレジット（作曲・作詞・編曲）まわりの立て直し。過去に保存されてしまった壊れた値を戻せるようにし、同じ動画を何度も調べ直す無駄を止め、件数が多いときに画面が固まるのを直した。新しい権限は要求していない。",
    "points": [
      "保存済みの壊れたクレジット値を空欄へ戻せるようにした。値を保存する前の検査を入れる以前は、URL・権利表示・説明文の切れ端などがそのまま作曲者名などとして保存されることがあった。これまでの補完処理は空欄しか埋めないため、いったん入った誤った値は誰も上書きできず残り続けていた。履歴画面の「データ管理・一括処理」→「データ修復」（既定では畳んである）から、下見（何件・どの値を消すかの確認）→実行の順で戻せる。",
      "下見を通さずに実行することはできない。下見が使い捨ての引換券を出し、実行はそれを要求する。券には対象の中身の指紋が入っているので、件数が同じでも対象が入れ替わっていれば止まる。実行時にも件数を数え直し、食い違えば1件も書かずに中止する。",
      "消した元の値は各動画の記録として残るのでバックアップにも入り、「元に戻す」で書き戻せる。書き戻すのは空欄の役割だけなので、修復後に正しく補完された値を古い値で上書きしない。",
      "実行直後に自己点検（読み取りだけ）が走り、消し残しと消しすぎの両方向を確認する。ただし「何を不正とみなすか」の判定基準そのものは点検できない旨も画面に出す。",
      "手で入力した値は修復の対象外。概要欄から読んだ生データ・最終チェック日時には触れない。",
      "概要欄から情報が取れなかった動画を毎月呼び戻すのをやめた。「作曲・作詞・編曲のどれかが空」だけを条件にしていたため、そもそも概要欄にその記載が無い動画（一般チャンネルでは実測で約95%）が30日ごとに丸ごと対象へ戻っていた。空のまま終わった回数に応じて次に調べる間隔を 30日 → 180日 → 360日 → 720日 と延ばし、期日も動画ごとにずらして一斉復活を防ぐ。手元のデータ32,538件で対象が3,511件から27件になった。何か1つでも埋まれば間隔は30日へ戻る。",
      "その間隔設定の説明が実際と食い違っていたのを直し、押す前に読める場所へ出した。これまで説明はホバーしないと出ないツールチップにしかなく、「30日以内」という古い文面のままだった。実行前の確認ダイアログに今回この間隔待ちで外れた件数とやり直す方法を出し、対象が0件のときも「対象なし（間隔待ち N件）」と理由が読めるようにした。",
      ...
      "手動確認の一覧を開いた瞬間にブラウザが固まるのを直した。対象13,475件でカードを一度に全部組み立てており、12.5秒応答しなくなっていた。最初の50件だけ描き、下端まで送ると50件ずつ足す方式に揃えて0.06秒になった。保存済みの行は上限を超えても必ず表示する。",
      ...
      "保存しているデータの構造（DB_VERSION）は 5 のままで、既存のデータを作り替える処理は行わない。新しい権限は要求していない（permissions / host_permissions は v1.47.0 と同一）。外部への送信も増えていない。",
      "Test: 回帰ハーネス34本 0 failed。修復機能は独立レビュー2回（1回目は NO-GO・指摘2件を実装で解消）、変異23件すべて検出。",
      "実データでの適用実績: 自分の手元のデータで528件を空欄化し、修復前後のバックアップを全件突合して、正常な値の巻き込み0・消し残し0・記録528件すべてに元の値が残っていることを確認済み（2026-08-25）。",
  ...
  },
  {
    "version": "1.47.0",
    "date": "2026-08-25",
    "summary": "画面まわりの整理。よく使う操作とたまにしか使わない操作を分け、時間のかかる処理の進み具合が1か所で読めるようにした。できることの追加・削除はなく、置き場所と見え方の変更が中心。",
    "points": [
      ...
      "履歴画面のメンテナンス操作を折り畳みへ入れた（「データ管理・一括処理」）。既定は畳んだ状態で、開いたか閉じたかは次に開いたときも覚えている。進捗表示だけは折り畳みの外に残したので、畳んでいても走っている処理の状況が読める。処理の実行中は中止ボタンが隠れないよう、折り畳みを開いたまま固定する。",
      "時間のかかる処理の進み具合を1か所へまとめた。進捗バー・1行の状況メッセージ・直近5件の処理履歴を同じ場所に表示する。進み具合は保存されるので、タブを閉じて開き直しても「何が」「どこまで」進んで終わったのかが読める。",
      "タブを閉じたときの動きを揃えた。これまでは処理によって「止まる」ものと「見えないところで走り続ける」ものに分かれていたが、すべて中止に統一した。ただし「まとめて削除」は削除1件が終わった区切りでだけ止まるので、削除の途中で中断されることはない。中断された場合も、何件消したかは残る。",
      ...
      "Test: 回帰ハーネス31本 0 failed。画面の受入確認は実ブラウザ（Chromium）で34項目すべて通過。感応性＝該当箇所を1行崩すと対応する検査だけが FAIL し、戻すと全通過。",
      "実機での動作確認は実施済み。"
    ]
  },
  {
    "version": "1.46.0",
    "date": "2026-08-11",
    "summary": "実際にストアへ提出する版。v1.45.0（提出用に組んだが未提出のままだった版）に、その後 main へ入ったクレジット候補の生成をパネルで選んだプロフィールで動かす修正（cnd7）を足した。公開版 v1.42.8 からの差分はこの版に全部入っている。",
    "points": [
      ...
      "Test: 回帰ハーネス29本 0 failed。cnd7 分は tests/verify_official_search_filter_credits.js に REQ-1〜5 を追加（19 passed）。感応性＝呼び出し側の分離を潰す変異で REQ-1/REQ-2 の2件が FAIL・復元後 SHA 一致を確認。",
  ...
  },
  {
    "version": "1.45.0",
    "date": "2026-08-10",
    "summary": "実際にストアへ提出する版。v1.44.0（v1.43.x の修正まとめ）に、作業ブランチで作っていた後で見る整理（v1.43.12）を統合した。v1.44.0 と v1.43.12 は提出せずローカルに留めた版なので、公開版 v1.42.8 からの差分はこの版に全部入っている。",
    "points": [
      "後で見る（Watch Later）の整理を収載: 視聴済みDBとの照合スキャン・1件削除・一覧確認つきのまとめて削除。削除は「後で見る」から1件外す操作だけに絞ってあり、content script 側の中継が playlistId と操作種別を毎回検査して、それ以外の編集要求を拒否する。",
