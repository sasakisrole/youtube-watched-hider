...
## v1.50.0 (2026-09-03)
**寄付の導線を1本だけ足した**。無償で公開しているツールなので、役に立った人が任意で支援できる先（GitHub Sponsors）へのリンクを、ポップアップの一番下と「使い方・更新情報」ページの末尾に置いた。機能の追加・削除はなく、新しい権限も要求していない。
- **ポップアップの最下部に「GitHub Sponsors で開発を支援する」リンクを置いた**。設定の折り畳みの外なので開かなくても見えるが、1行のテキストリンクにとどめてバナーやポップアップでの割り込みはしない。
